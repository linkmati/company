...
namespace CompanyFeeds.Web.Controllers
{
	...
	public class BaseController : Controller
	{
		...
		#region Cookies Helper
		private CookiesHelper _cookies;
		/// <summary>
		/// Gets the cookies helper
		/// </summary>
		public CookiesHelper Cookies
		{
			get
			{
				if (_cookies == null)
				{
					if (HttpContext != null)
					{
						_cookies = new CookiesHelper(HttpContext);
					}
					else
					{
						throw new NullReferenceException("Controller context not set.");
		...
		public SiteConfiguration SiteConfiguration
		{
			get
			{
				...
				{
					_siteConfiguration = Cache.SiteConfiguration;
				}
		...
		public SpamPreventionConfiguration SpamPreventionConfig
		{
			get;
			set;
		}
		...
		{
			SpamPreventionConfig = SpamPreventionConfiguration.Current;
			//Ensure Cached Items
			EnsureCachedItems();
...
//			    string receivedEx = null;
//			    string reason = null;
//			    try
//			    {
//			        reason = SecurityHelper.ReadMemberCookie(HttpContext.Request.Cookies, HttpContext.Response.Cookies, Session);
//			    }
//			    catch (Exception ex)
//			    {
//			        receivedEx = ex.Message;
//                }
//                var cookie = Request.Cookies["member"];
//                if (cookie != null && cookie.Values["id"] == "30")
//                {
//                    if (reason != null)
//                    {
...
//                }
		...
		protected void AddErrors(ModelStateDictionary modelState, ValidationException ex)
		{
			foreach (ValidationError error in ex.ValidationErrors)
			{
				...
				modelState.SetModelValue(error.FieldName, ValueProvider[error.FieldName]);
			}
		} 
		...
		{
			return ResultHelper.RssResult(title, url, description, dataSource, itemTitleProperty, itemUrlProperty, itemDescriptionProperty, itemDateProperty, urlGetHandler);
		}
