...
namespace CompanyFeeds
{
	public static class TextUtils
	{
		...
		public static string CleanHtml(string value)
		{
			...
			//Replace links
			value = Regex.Replace(value, "<a[^>]+href=\"?'?([^'\">]+)\"?'?[^>]*>(.*?)</a>", "<a href=\"$1\" rel=\"nofollow\" target=\"_blank\">$2</a>", RegexOptions.IgnoreCase | RegexOptions.Singleline);
		...
		/// <summary>
		/// Extract links from html extract and returns a list of urls
		/// </summary>
		/// <returns></returns>
		public static List<string> ExtractLinks(string value)
		{
			var urls = new List<string>();
			Regex regex = new Regex("<a[^>]+href=\"?'?([^'\">]+)\"?'?[^>]*>", RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);
