...
namespace CompanyFeeds.Web.Controllers
{
	public class UsersController : BaseController
	{
			...
			try
			{
				...
				if (GetErrors(ModelState).Count == 0)
				{
					...
					else
					{
						...
						NotificationsService.SendValidationMail(user, Path.Combine(SiteConfiguration.GetApplicationRootConfigurationPath(), SiteConfiguration.Mail.TemplatesPath["Welcome"]), SiteConfiguration.Mail.AdminMailAddress, SiteConfiguration.Mail.AdminMailName, SiteConfiguration.Mail.Subjects["Welcome"], validationUrl, SiteConfiguration.Mail.SmtpServer);
		...
		{
			UserState user = SecurityHelper.Login(Session, HttpContext.Request.Cookies, Email, Password);
			if (user != null)
			{
				//redirect
				if (returnUrl != null)
