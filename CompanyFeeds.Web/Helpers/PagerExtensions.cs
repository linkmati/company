...
namespace CompanyFeeds.Web.Helpers
{
	...
	public class Pager
	{
		...
		public string RenderHtml()
		{
			...
			{
				sb.Append(GeneratePageLink(this.LabelPrevious, this.currentPage - 1, new{@class="previous"}));
			}
			else
			{
			...
			if (this.currentPage < pageCount-1)
			{
				...
				sb.Append(GeneratePageLink(this.LabelNext, (this.currentPage + 1), new{@class="next"}));
			}
			else
			{
		...
		private string GeneratePageLink(string linkText, int pageIndex, object htmlAttributes)
		{
			...
			return this._htmlHelper.ActionLinkFormatted(linkText, null, this._htmlHelper.ViewContext.RouteData.Values["action"].ToString(), this._htmlHelper.ViewContext.RouteData.Values["controller"].ToString(), values, new RouteValueDictionary(htmlAttributes));
		}
	}
}
